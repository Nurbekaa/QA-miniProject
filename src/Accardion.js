...
import KeyboardArrowUpOutlinedIcon from '@mui/icons-material/KeyboardArrowUpOutlined'; //стрелка вверх
import KeyboardArrowDownOutlinedIcon from '@mui/icons-material/KeyboardArrowDownOutlined';// стрелка вниз
...
export function Acc(){
    ...
    return (
        <div className="section">
            <h1 className='section_title'>FAQ about React useState</h1>
            {questions.map((item) => (<Qusetion items={item} />))}
        </div>
    )
}
...
function Qusetion({items}){
    ...
    return(
        <div className='container'>
            <div className='container_for_qusetion'>
                <p className='question'><b className='number_for_question'>{id}</b>. {question}</p>
                <button className='hide_btn' onClick={showHide}>{hide ? <KeyboardArrowUpOutlinedIcon/>: <KeyboardArrowDownOutlinedIcon/> }</button>
            </div>
            <div className='container_for_answer'>
                <p className='answer'>{hide && answer}</p>
            </div>
        </div>
    )
}
